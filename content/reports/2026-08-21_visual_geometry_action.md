...
检索日期：2026-08-21。范围：2024-01-01 至检索日，优先 2025–2026 新作。检索从 arXiv、CVF、作者项目页与代码仓库获得线索，并以论文正文、正式出版页和官方资源为准。库中已有 FlowWAM（2607.13017）与 Masked Visual Actions（2607.19343），本报告不重复收录。

本专题所说的 action representation 不是“给策略额外加入深度输入”，而是把动作本身或动作引起的变化改写为视频模型原生、跨本体可共享的视觉/几何接口。当前形成四条路线：

- **光流与 latent motion**：Motus、Latent Policy Steering 将无标签视频中的光流压缩为动作接口。
- **骨架、渲染与视觉场**：VAP、OSCAR、EA-WM、GeniWorld、iMaC、RoFacto 把关节动作经运动学投影为图像、骨架、深度或接触距离场。
...
## 2. Latent Policy Steering with Embodiment-Agnostic Pretrained World Models
...
## 4. OSCAR: Omni-Embodiment Action-Conditioned World Model for Robotics
...
## 7. PointAction: 3D Points as Universal Action Representations for Robot Control
...
## 9. Robot-Factored World Models via Robot Rendering
...
## 10. TraceGen: World Modeling in 3D Trace Space Enables Learning from Cross-Embodiment Videos
...
## 11. Scaling Cross-Embodiment World Models for Dexterous Manipulation
...
## 12. μ₀: A Scalable 3D Interaction-Trace World Model
